...
import { Zap, Clock, AlertCircle, Check, BarChart2, Tag, Activity, FileCode } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-text-primary flex items-center gap-2">
            <Activity className="w-6 h-6 text-accent-primary" /> Fortnightly Performance Digest History
          </h1>
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            disabled={triggering}
            className="px-5 py-2.5 bg-accent-primary text-bg-base rounded-lg text-sm font-mono font-bold hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center space-x-2 shadow-soft"
          >
            <span className="inline-flex items-center gap-2">
              {triggering ? (
                <><Clock className="w-4 h-4 animate-spin" /> Generating Digest...</>
              ) : (
                <><Zap className="w-4 h-4" /> Trigger New Digest</>
              )}
            </span>
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-panel backdrop-blur shadow-soft rounded-xl border border-accent-primary/20 p-12 text-center space-y-4">
          ...
          <p className="text-sm text-text-muted max-w-md mx-auto font-body">
          ...
          <button
            onClick={handleTriggerDigest}
            disabled={triggering}
            className="px-5 py-2.5 bg-accent-primary text-bg-base font-mono font-bold text-sm rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50 inline-flex items-center gap-2 shadow-sm"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 bg-panel backdrop-blur shadow-soft rounded-xl border border-accent-primary/20 p-6 space-y-4">
            <h2 className="text-xs font-mono uppercase tracking-wider text-text-muted mb-2 flex items-center gap-1.5">
              <Activity className="w-4 h-4 text-accent-primary" /> Stored Digests ({digests.length})
            </h2>
              ...
              {digests.map((d) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-xl text-sm transition-all flex flex-col space-y-2 border ${
                      isSelected
                  ...
                  >
                    ...
                    <div className="grid grid-cols-3 gap-1.5 text-[11px] font-mono">
                      <div className="bg-bg-base/60 p-1.5 rounded text-center border border-accent-primary/10">
          ...
          {/* Selected Digest Detail View Panel */}
          <div className="lg:col-span-2 space-y-6">
            {selectedDigest ? (
              <>
                {/* Header Information Card */}
                <div className="bg-panel backdrop-blur shadow-soft rounded-xl border border-accent-primary/20 p-6 space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-accent-primary/20 pb-4 gap-2">
                    <div>
                  ...
                  {/* Metric Summary Cards styled with bg-panel backdrop-blur shadow-soft rounded-xl border border-accent-primary/20 p-6 */}
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                    <div className="bg-panel backdrop-blur shadow-soft rounded-xl border border-accent-primary/20 p-6 space-y-1">
                ...
                {/* Top Recurring Question Themes Panel styled with bg-panel backdrop-blur shadow-soft rounded-xl border border-accent-primary/20 p-6 */}
                <div className="bg-panel backdrop-blur shadow-soft rounded-xl border border-accent-primary/20 p-6 space-y-4">
                  <h3 className="text-sm font-bold font-mono text-accent-primary uppercase tracking-wider flex items-center gap-2">
                      ...
                      {selectedDigest.summary.top_query_themes.map((t, idx) => {
                        if (typeof t === 'object' && t !== null && 'theme' in t) {
                          return (
                            <div
                              key={idx}
                              className="px-3.5 py-2 rounded-xl bg-panel backdrop-blur border border-accent-primary/30 text-xs font-mono flex items-center space-x-2 text-text-primary shadow-sm"
                            >
                              <span className="font-semibold text-accent-primary">#{t.theme}</span>
                              <span className="px-2 py-0.5 rounded-full bg-accent-primary/10 text-accent-primary border border-accent-primary/20 text-[10px]">
                                {t.count} queries
                              </span>
                            </div>
                          );
                        ...
                        return (
                          <div
                            key={idx}
                            className="px-3.5 py-2 rounded-xl bg-panel backdrop-blur border border-accent-primary/30 text-xs font-mono text-accent-primary shadow-sm"
                          >
                            {String(t)}
                          </div>
                        );
                ...
                {/* Raw Summary JSON Viewer Panel styled with bg-panel backdrop-blur shadow-soft rounded-xl border border-accent-primary/20 p-6 */}
                <div className="bg-panel backdrop-blur shadow-soft rounded-xl border border-accent-primary/20 p-6 space-y-3">
                  <h3 className="text-xs font-mono text-text-muted uppercase flex items-center gap-1.5">
                    <FileCode className="w-4 h-4 text-accent-primary" /> Summary JSON Output
                  </h3>
                  <pre className="p-4 rounded-lg bg-bg-base/80 border border-accent-primary/20 text-xs font-mono text-accent-primary overflow-x-auto">
                    {JSON.stringify(selectedDigest.summary || {}, null, 2)}
                  </pre>
                </div>
              </>
            ) : (
              <div className="bg-panel backdrop-blur shadow-soft rounded-xl border border-accent-primary/20 p-12 text-center text-text-muted font-mono text-sm">
